...
# ─── CONFIG ─────────────────────────────────────────────────────────
ROOT_DIR = r"C:\Users\Nighil Natarajan\ckmam_proj\dcase_2026"
#CHECKPOINT_DIR = "checkpoints_dcase2026_near_june12_att3"

# Fine tuned Checkpoint (all development dataset + addnl training set)
CHECKPOINT_DIR = "checkpoints_att3_finetuned_june13"
...
def main():
    ...
    for m in ALL_MACHINE_TYPES:
        ...
        for dom, trE, teE, labels in [
            ...
            if dom == "source":
                ...
                source_scores = DomainModel(
                    **best["cfg"]
                )

                source_scores.fit(trE)

                _, src_scores = source_scores.predict(teE)

            else:
                target_auc = best["auc"]

                target_scores = DomainModel(
                    **best["cfg"]
                )

                target_scores.fit(trE)

                _, tgt_scores = target_scores.predict(teE)
        ...
        all_source_auc.append(source_auc)
        all_target_auc.append(target_auc)
        all_pauc.append(pauc)
        ...
        print(
            f"  MACHINE OMEGA={omega:.4f}"
        )

    official_omega = harmonic_mean(
        all_source_auc +
        all_target_auc +
        all_pauc
    )
    ...
    print("\n==============================")
    print(f"FINAL OMEGA = {official_omega:.4f}")
    print("==============================")
