...
class ASTRA_AttnPatchRGBDataset(Dataset):
    def __init__(self, root_dir, machine_type, split="train",
        ...
        #print(f"{machine_type} | global_attr_dim={self.global_attr_dim}")

        folder = os.path.join(root_dir, machine_type, f"{split}RGB_NEAR")
        self.samples = sorted(glob(os.path.join(folder, "*.png")))

        # attributes
        self.machine_attr_dim = 0
        #self.machine_attr_dim = global_attr_dim
        self.attr_map = {}
        if machine_type in USE_ATTR:
            ...
            if os.path.isfile(csv_p):
                ...
                #df["basename"] = (df[fcol].astype(str).apply(lambda x: os.path.splitext(os.path.basename(x))[0]))
    ...
    def __getitem__(self, idx):
        ...
        #attrs = self.attr_map.get(fname,torch.zeros(self.global_attr_dim))

        attrs = self.attr_map.get(fname,torch.zeros(self.machine_attr_dim))
        
        # if idx == 0:
        #     print(
        #         f"{self.machine_type} | fname={fname} | "
        #         f"attr_shape={attrs.shape} | "
        #         f"machine_attr_dim={self.machine_attr_dim}"
        #     )
